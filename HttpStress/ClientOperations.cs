    ...
    public static class ClientOperations
    {
        ...
        public static (string name, Func<RequestContext, Task> operation)[] Operations =>
            new (string, Func<RequestContext, Task>)[]
            {
                ...
                async ctx =>
                {
                    ...
                    {
                        var exc = e;
                        if (e is HttpRequestException hre && (hre.InnerException is IOException || hre.InnerException is WebException))
                        {
                            exc = hre.InnerException;
                        }
                ...
                async ctx =>
                {
                    ...
                    string checksumMessage = ValidateServerChecksum(m.Headers, checksum, required: false) ? "server checksum matches client checksum" : "server checksum mismatch";
                    ValidateContent(content, await m.Content.ReadAsStringAsync(), checksumMessage);
                }),
                ...
                async ctx =>
                {
                    ...
                    bool isValidChecksum = ValidateServerChecksum(m.Headers, checksum, required: false);
        ...
        private static void ValidateContent(string expectedContent, string actualContent, string? details = null)
        {
            if (actualContent != expectedContent)
            {
                int divergentIndex = 
                    Enumerable
                        .Zip(actualContent, expectedContent, (first, second)=> Tuple.Create(first,second))
        ...
        private sealed class ByteAtATimeNoLengthContent : HttpContent
        {
            ...
            protected override async Task SerializeToStreamAsync(Stream stream, TransportContext? context)
            {
                ...
                {
                    await stream.WriteAsync(_buffer, i, 1);
