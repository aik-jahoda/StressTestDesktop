...
public static class Program
{
    ...
    public static async Task<int> Main(string[] args)
    {
        ...
        return (int)await Run(new Configuration()
        {
            ...
            ConcurrentRequests = Environment.ProcessorCount,
            RandomSeed = new Random().Next(),
             MaxContentLength = 1000,
                   MaxRequestUriSize = 5000,
                    MaxRequestHeaderCount =90,
                    MaxRequestHeaderTotalSize = 1000,
            //        OpIndices = cmdline.ValueForOption<int[]>("-ops"),
            //        ExcludedOpIndices = cmdline.ValueForOption<int[]>("-xops"),
                    MaxParameters = 1,
                    DisplayInterval = TimeSpan.FromSeconds(60),
    ...
    private static async Task<ExitCode> Run(Configuration config)
    {
        ...
        string GetAssemblyInfo(Assembly assembly) => $"{assembly.Location}, modified {new FileInfo(assembly.Location).LastWriteTime}, version {assembly.FullName}";
